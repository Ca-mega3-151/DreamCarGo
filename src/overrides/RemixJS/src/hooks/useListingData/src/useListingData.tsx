...
import { useFetcher, useSearchParams } from '~/overrides/@remix-run/react';
...
export const useListingData = <Model extends AnyRecord>({
  ...
  useEffect(() => {
    if (fetcherData.data) {
      ...
      if (isEmpty(fetcherData.data.info.hits) && !!page && page !== fetcherData.data.page.toString()) {
  ...
  useEffect(() => {
    if (loaderData) {
      const page = currentSearchParams.get('page');
      if (isEmpty(loaderData.info.hits) && !!page && page !== loaderData.page.toString()) {
