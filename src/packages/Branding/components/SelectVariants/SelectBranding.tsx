...
import { SelectSingleDecouplingWithCreatable } from '~/components/SelectDecouplingWithCreatable';
import { notification, SelectSingleDecouplingProps } from '~/shared/ReactJS';
import { handleCatchClauseAsMessage } from '~/utils/functions/handleErrors/handleCatchClauseSimple';

interface Props
  extends Pick<
    SelectSingleDecouplingProps<Branding, Branding['brandingCode']>,
    'disabled' | 'allowClear' | 'placeholder' | 'onChange'
  > {
  branding?: Branding['brandingCode'];
}
...
export const SelectBranding = ({ branding, ...props }: Props) => {
  const { t } = useTranslation(['branding']);

  const FormCreate = useMemo(() => {
    return 'SelectBranding__FormCreate' + v4();
  }, []);
  ...
  return (
    <>
      <SelectSingleDecouplingWithCreatable<Branding, Branding['brandingCode'], BrandingFormMutationValues>
        {...props}
        searchType="client"
        value={branding}
        ...
        formUid={FormCreate}
        Form={({ onSubmit, isSubmitting, formUid }) => {
          return (
