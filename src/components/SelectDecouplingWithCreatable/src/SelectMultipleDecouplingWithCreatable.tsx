...
export const SelectMultipleDecouplingWithCreatable = <
...
>({
  ...
  const [serviceResponse, setServiceResponse] = useState<Model[]>([]);
  ...
  const { handleCreate, isCreating, isOpenModalCreate, handleCloseModalCreate, handleOpenModalCreate } = useCreate({
    create: async data => {
      ...
      if (option) {
        ...
        onChange?.(
          nextValue,
          serviceResponse.reduce<SelectOption<ModelId, Model>[]>((result, item) => {
            const itemInOption = transformToOption(item);
            if (itemInOption && nextValue.includes(itemInOption?.value)) {
              return model ? result.concat(itemInOption) : result;
            }
            return result;
          }, []),
        );
  ...
  return (
    <>
      <SelectMultipleDecoupling<Model, ModelId>
        {...props}
        value={value}
        initializingText={() => {
